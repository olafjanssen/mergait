...
class RawReader:
    ...
    def update_with(self, msg):
        ...
        if 't' in msg:
            t = msg['t']
            self.t_range = [t, t
                            ] if self.t_range == [] else [self.t_range[0], t]
        ...
        if msg['type'] == 'iPhone-pedo':
            self.phone_activity.append({
                ...
                msg['pedo']['activity'],
                'speed':
                0 if msg['pedo']['pace'] == 0 else 1.0 / msg['pedo']['pace'],
                'step':
                msg['pedo']['step'],
                'cadence':
                msg['pedo']['cadence'] * 60,
                'floors_ascended':
                msg['pedo']['floorsAscended']
                if 'floorsAscended' in msg['pedo'] else 0,
                'floors_descended':
                msg['pedo']['floorsDescended']
                if 'floorsDescended' in msg['pedo'] else 0
            })
        ...
        if msg['type'] == 'iPhone-motion':
            ...
            a_vert = msg['motion']['a'][0] * gx + msg['motion']['a'][
                1] * gy + msg['motion']['a'][2] * gz
        ...
        if msg['type'] == 'iPhone-location':
            self.phone_location.append({
                't':
                pd.Timestamp(msg['location']['timestamp'], unit='s'),
                'lon':
                msg['location']['coordinate']['lon'],
                'lat':
                msg['location']['coordinate']['lat'],
                'lonlat_acc':
                msg['location']['coordinate']['acc'],
                'alt':
                msg['location']['altitude']['val'],
                'alt_acc':
                msg['location']['altitude']['acc'],
                'course':
                msg['location']['course']['val'],
                'course_acc':
                msg['location']['course']['acc'],
                'speed':
                msg['location']['speed']['val'],
                'speed_acc':
                msg['location']['speed']['acc'],
            })
        ...
        if msg['type'] == 'Spotify' and 'playstate' in msg:
            ...
            self.music.append({
                ...
                msg['playstate']['uri'],
                'paused':
                msg['playstate']['paused'],
                'artist':
                msg['playstate']['artist'],
                'track':
                msg['playstate']['track'],
                'context_uri':
                msg['playstate']['contextUri'],
                'context':
                msg['playstate']['contextTitle'],
                'position':
                msg['playstate']['position'] / 1000,
                'repeat_mode':
                msg['playstate']['repeatMode']
                if 'repeatMode' in msg else 'off',
                'shuffle':
                msg['playstate']['shuffle'] if 'shuffle' in msg else False,
                'crossfade':
                msg['playstate']['crossfadeState']
                if 'crossfadeState' in msg else False,
            })
