...
def extract_bouts(df,
    ...
    bouts = groups.agg({
        range_column: ['count', 'min', 'max'],
        '_filter': ['first']
    })
    bouts.columns = [
        'count', range_column + '_start', range_column + '_end', valid_column
    ]
...
def with_padded_bout_window(bouts, window=[0, 0], range_column='t'):
    ...
    bouts[range_column +
          '_start'] = bouts[range_column + '_start'] + pd.to_timedelta(
              window[0], unit='s')
    bouts[range_column +
          '_end'] = bouts[range_column + '_end'] + pd.to_timedelta(window[1],
                                                                   unit='s')

    return bouts


def add_bouts_as_column(df,
                        bouts,
                        new_column='bout',
                        range_column='t',
                        valid_column='valid',
                        value='column',
                        reset_value=pd.Series()):
    '''
    ...
    for idx, bout in bouts.iterrows():
        df.loc[(df[range_column] >= bout[range_column + '_start']) & (
            df[range_column] < bout[range_column + '_end']
        ), new_column] = bout[
            valid_column] if value == 'column' else idx if value == 'index' else value

    return df


def interpolate_bouts_as_column(df,
                                df_values,
                                bouts,
                                new_column='bout',
                                range_column='t',
    ...
    for idx, bout in bouts.iterrows():
        bout_data = df[(df[range_column] >= bout[range_column + '_start'])
                       & (df[range_column] < bout[range_column + '_end'])]

        t_data = pd.to_numeric(bout_data[range_column])
        t_bout, val_bout = pd.to_numeric(
            df_values[range_column]), df_values[value_column]
...
def select_range(df, window, range_column='t', include_end=True):
    ...
    return [
        dff[(dff[range_column] >= window[0])
            & (dff[range_column] <= window[1])] if include_end else
