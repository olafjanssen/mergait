...
def filter_to_valid_bouts_recipe(df,
                                 df_music,
                                 df_phone_activity,
                                 df_sessions,
                                 sections=None):
    ...
    valid = ~(df['bad_not_running'] | df['bad_not_flat'] | df['bad_no_music']
              | df['bad_half_step'])
...
def recipe_footpod_symmetry(df_footpods,
                            df_music,
                            df_phone_activity,
                            df_sessions,
                            sections=None):
    ...
    df_pods = pd.DataFrame(df_footpods)

    # combine pod data into pod_gait and annotate bad steps
    df_pod_steps = merge_left_right_data(df_pods)

    df_pod_steps = filter_to_valid_bouts_recipe(df_pod_steps,
                                                df_music,
                                                df_phone_activity,
                                                df_sessions,
                                                sections=sections)

    append_symmetry_index(df_pod_steps, method='sa')
...
def recipe_imu_symmetry(df_imu,
                        df_music,
                        df_phone_activity,
                        df_sessions,
                        sections=None):
    ...
    '''
    log.debug(
        '[ Computing symmetry information from imu vertical acceleration')

    df_acc = pd.DataFrame(df_imu)
    ...
    df_imu_steps = merge_left_right_data(df_imu_steps,
                                         feet=['A', 'B'],
                                         side='both')

    df_imu_steps = filter_to_valid_bouts_recipe(df_imu_steps,
                                                df_music,
                                                df_phone_activity,
                                                df_sessions,
                                                sections=sections)

    append_symmetry_index(df_imu_steps, method='sa')
    ...
    df_gsi_summary = df_gsi_bouts.drop('bout_idx', axis=1).groupby(
        by=by_bouts, sort=False).agg(['mean', 'median'])
...
def compute_gsi_from_imu_recipe(df,
                                df_imu,
                                by=['track_uri', 'session_id', 'section']):
    '''
    ...
    '''
    log.debug(
        '[ Following recipe to compute Gait Symmetry Index per running bout')
    ...
    def apply_gsi(row):
        df_acc = df_imu[(df_imu.t > row.t[0])
                        & (df_imu.t < row.t[1])]
    ...
    df_gsi_bouts[['gsi', 'stride_duration',
                  'cadence']] = df_gsi_bouts.apply(apply_gsi,
                                                   axis=1,
                                                   result_type='expand')
