...
def merge_music_playstate(df, df_music, columns=['t', 'track_uri']):
    ...
    df_music['__temp'] = (df_music.track_uri !=
                          df_music.track_uri.shift()).cumsum()
    df_music_songs = df_music.groupby(by=['__temp'], sort=False)

    # merge simple properties
    df = pd.merge_asof(df,
                       df_music[['t', 'track_uri']],
                       on='t',
                       direction='backward')
    ...
    for idx in range(len(df_music_songs)):
        ...
        next_song = False if is_last else df_music_songs.get_group(keys[idx +
                                                                        1])
        ...
        add_bouts_as_column(df,
                            song_bouts,
                            new_column='bad_no_music',
                            reset_value=True)
        # interpolate and extrapolate play position
        interpolate_bouts_as_column(df,
                                    song,
                                    song_bouts,
                                    new_column='position',
...
def append_music_section(df,
    ...
    df = pd.merge_asof(df.sort_values(by='position'),
                       df_sections[['uri', 'start',
                                    'section']].sort_values(by='start'),
                       left_on='position',
                       right_on='start',
                       left_by=['track_uri'],
                       right_by='uri',
...
def get_music_features_per_section(df_features,
                                   df_sections,
                                   df_segments,
                                   track_uris=None):
    '''
    ...
    if track_uris is None:
        ...
        df = df_sections[df_sections['uri'].isin(track_uris)]

    df = df.merge(df_features, on='uri', suffixes=[None, '_track'])
    df.drop(['type', 'id', 'track_href', 'analysis_url'], axis=1, inplace=True)
    df.sort_values(by='start', inplace=True)

    # add segment, pitch and timbre information
    df_segs = df_segments[df_segments['uri'].isin(
        df['uri'])].sort_values(by='start')
    df = pd.merge_asof(df_segs[['uri', 'start', 'pitches', 'timbre']],
                       df,
                       on='start',
                       by='uri',
    ...
    df = df.groupby(['uri', 'section']).agg(agg_funs)
    df.columns = df.columns.map(
        lambda x: '_'.join(a for a in x if a != 'first'))
...
def get_music_features_per_track(df_features, df_segments, track_uris=None):
    ...
    if track_uris is None:
        ...
        df = df_features[df_features['uri'].isin(track_uris)]

    df.drop(['id', 'track_href', 'analysis_url'], axis=1, inplace=True)

    # add segment, pitch and timbre information
    df_segs = df_segments[df_segments['uri'].isin(df['uri'])]
    df = df_segs[['uri', 'pitches',
                  'timbre']].merge(df, on='uri', suffixes=['_segment', None])
    ...
    df = df.groupby(['uri']).agg(agg_funs)
    df.columns = df.columns.map(
        lambda x: '_'.join(a for a in x if a != 'first'))
    df.reset_index(drop=True, inplace=True)
...
def get_music_track_info(df_tracks, df_artists, track_uris):
    ...
    if track_uris is None:
        ...
        df = df_tracks[df_tracks['uri'].isin(track_uris)]
        ...
        df = df[[
            'artists', 'artist_uri', 'name', 'popularity', 'album_name',
            'album_uri', 'album_images'
        ]]
        df.rename(columns={
            'artists': 'artist_name',
            'album_images': 'cover_url'
        },
            inplace=True)

        df = df.merge(
            df_artists[['uri', 'popularity', 'genres', 'followers_total']],
            left_on='artist_uri',
            right_on='uri',
