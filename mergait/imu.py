...
def gait_features_from_vertical_acceleration(timestamps,
                                             a_vert,
                                             contact_time_range=[50, 200],
                                             step_time_range=[200, 1000]):
    '''
    ...
    log.debug('Found {} IC and {} FC peaks'.format(len(ic_times_ns),
                                                   len(fc_times_ns)))
    ...
    df = df[df['t'].shift(-1) - df['t'] >
            min_step_time]  # double ic's lead to invalid step times
    df = df[df['step_duration'] >
            df['contact_time']]  # contact time cannot be longer than step time

    df['stride_duration'] = pd.to_numeric(df['t'].shift(-2) - df['t']) / 1e6
    df = df[df['stride_duration'] > 0]  # stride duration must be positive
    df = df[df['t'].shift(-2) - df['t'] <
            2 * max_step_time]  # also enforce maximum stride time

    df['flight_ratio'] = (df['step_duration'] -
                          df['contact_time']) / df['step_duration']

    # drop extra columns and do clean up
    df.drop(['t_contact_min', 't_step_min', 't_ic', 't_opp', 't_ic_opp'],
            axis=1,
            inplace=True)
...
def gait_symmety_index_from_acceleration(ax,
                                         ay,
                                         az,
                                         maxlag=150,
                                         deadlag=50,
